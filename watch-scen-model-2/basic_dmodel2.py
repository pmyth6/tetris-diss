...
class Model:
    def __init__(self):
        #load model
        self.model = keras.models.load_model("GS80N001LR.keras",
    ...
    def model_play(self, current_move):
        ...
        if int(move_index1) <=9 and int(move_index2)>9:
            action1 = self.moves[int(move_index2)]
            action2 = self.moves[int(move_index1)]
        else:
